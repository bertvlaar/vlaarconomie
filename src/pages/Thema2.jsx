// src/pages/Thema2.jsx
//
// Pagina-component voor Thema 2 — Gaming, Skins & In-App Purchases.
// Wordt gerenderd binnen <ThemaLayout /> (zie App.jsx: route "/thema-2").
// Haalt alle inhoud op uit src/data/thema2Data.js en toont daarnaast, indien
// beschikbaar, de context/koppeling uit src/data/themas.js via useOutletContext.

import { useMemo, useState } from 'react'
import { Link, useOutletContext } from 'react-router-dom'
import thema2Data from '../data/thema2Data.js'

export default function Thema2() {
  const outletContext = useOutletContext() || {}
  const { thema } = outletContext

  const { titel, beschrijving, paragrafen, begrippen, oefenvragen } = thema2Data
    ...
    <section className="space-y-10 pb-16">
      ...
      <div>
        <span className="font-mono text-xs font-semibold uppercase tracking-[0.2em] text-flashdark">
          Thema {thema?.id ?? 2} · Consumentengedrag &amp; Prijsmechanismen
        </span>
        ...
        {aantalBeantwoord === totaalVragen && (
          <div className="rounded-xl border border-border bg-header p-6 text-white">
            ...
            <p className="mt-1 text-sm text-slate-300">
              Onthoud: als een spel je op elke hoek een koopmoment, een aftellende klok of een
              mysterie-kistje voorschotelt, is dat geen toeval — dat is ontworpen. Klik op
