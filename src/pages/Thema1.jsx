// src/pages/Thema1.jsx
//
// Pagina-component voor Thema 1 — Scams, Crypto-bros & Get-Rich-Quick.
// Wordt gerenderd binnen <ThemaLayout /> (zie App.jsx: route "/thema-1").
// Haalt alle inhoud op uit src/data/thema1Data.js en toont daarnaast, indien
// beschikbaar, de context/koppeling uit src/data/themas.js via useOutletContext.

import { useMemo, useState } from 'react'
import { Link, useOutletContext } from 'react-router-dom'
import thema1Data from '../data/thema1Data.js'

export default function Thema1() {
  const outletContext = useOutletContext() || {}
  const { thema } = outletContext

  const { titel, beschrijving, paragrafen, begrippen, oefenvragen } = thema1Data
    ...
    <section className="space-y-10 pb-16">
      ...
      <div>
        <span className="font-mono text-xs font-semibold uppercase tracking-[0.2em] text-flashdark">
          Thema {thema?.id ?? 1} · Financiële geletterdheid &amp; Oplichting
        </span>
        ...
        {aantalBeantwoord === totaalVragen && (
          <div className="rounded-xl border border-border bg-header p-6 text-white">
            ...
            <p className="mt-1 text-sm text-slate-300">
              Onthoud: als iets gegarandeerd, razendsnel én zonder risico rijk maakt, is het
              geen investering — het is een verkooppraatje. Klik op "Opnieuw beginnen" om de
