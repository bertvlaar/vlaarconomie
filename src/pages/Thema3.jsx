// src/pages/Thema3.jsx
//
// Pagina-component voor Thema 3 — Abonnementenroof: Van Netflix tot HelloFresh.
// Wordt gerenderd binnen <ThemaLayout /> (zie App.jsx: route "/thema-3").
// Haalt alle inhoud op uit src/data/thema3Data.js en toont daarnaast, indien
// beschikbaar, de context/koppeling uit src/data/themas.js via useOutletContext.

import { useMemo, useState } from 'react'
import { Link, useOutletContext } from 'react-router-dom'
import thema3Data from '../data/thema3Data.js'

export default function Thema3() {
  const outletContext = useOutletContext() || {}
  const { thema } = outletContext

  const { titel, beschrijving, paragrafen, begrippen, oefenvragen } = thema3Data
    ...
    <section className="space-y-10 pb-16">
      ...
      <div>
        <span className="font-mono text-xs font-semibold uppercase tracking-[0.2em] text-flashdark">
          Thema {thema?.id ?? 3} · De Abonnementsmaatschappij
        </span>
        ...
        {aantalBeantwoord === totaalVragen && (
          <div className="rounded-xl border border-border bg-header p-6 text-white">
            ...
            <p className="mt-1 text-sm text-slate-300">
              Onthoud: een abonnement van "maar €9,99 per maand" is nooit alleen €9,99 — het is
              een vaste last die doorloopt tot jij hem actief stopzet. Klik op "Opnieuw beginnen"
              om de quiz nog eens te maken.
            </p>
          </div>
        )}
      </div>
