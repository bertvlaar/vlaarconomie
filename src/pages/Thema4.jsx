// src/pages/Thema4.jsx
//
// Pagina-component voor Thema 4 — Fast-fashion, Greenwashing & De kledingkast
// van de toekomst.
// Wordt gerenderd binnen <ThemaLayout /> (zie App.jsx: route "/thema-4").
// Haalt alle inhoud op uit src/data/thema4Data.js en toont daarnaast, indien
// beschikbaar, de context/koppeling uit src/data/themas.js via useOutletContext.

import { useMemo, useState } from 'react'
import { Link, useOutletContext } from 'react-router-dom'
import thema4Data from '../data/thema4Data.js'

export default function Thema4() {
  const outletContext = useOutletContext() || {}
  const { thema } = outletContext

  const { titel, beschrijving, paragrafen, begrippen, oefenvragen } = thema4Data
    ...
    <section className="space-y-10 pb-16">
      ...
      <div>
        <span className="font-mono text-xs font-semibold uppercase tracking-[0.2em] text-flashdark">
          Thema {thema?.id ?? 4} · Bedrijfseconomie &amp; Duurzaamheid
        </span>
        ...
        {aantalBeantwoord === totaalVragen && (
          <div className="rounded-xl border border-border bg-header p-6 text-white">
            ...
            <p className="mt-1 text-sm text-slate-300">
              Onthoud: een vage kreet als "eco" of "bewust" op een label is geen bewijs — pas een
              onafhankelijk gecontroleerd keurmerk maakt een duurzaamheidsclaim ook echt waar.
              Klik op "Opnieuw beginnen" om de quiz nog eens te maken.
            </p>
          </div>
        )}
      </div>
