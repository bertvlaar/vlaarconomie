// src/pages/Thema6.jsx
//
// Pagina-component voor Thema 6 — De Winstfabriek: Hoe runt een bedrijf een
// gezonde zaak? (Bedrijfseconomische kern).
// Wordt gerenderd binnen <ThemaLayout /> (zie App.jsx: route "/thema-6").
// Haalt alle inhoud op uit src/data/thema6Data.js en toont daarnaast, indien
// beschikbaar, de context/koppeling uit src/data/themas.js via useOutletContext.

import { useMemo, useState } from 'react'
import { Link, useOutletContext } from 'react-router-dom'
import thema6Data from '../data/thema6Data.js'

export default function Thema6() {
  const outletContext = useOutletContext() || {}
  const { thema } = outletContext

  const { titel, beschrijving, paragrafen, begrippen, oefenvragen } = thema6Data
    ...
    <section className="space-y-10 pb-16">
      ...
      <div>
        <span className="font-mono text-xs font-semibold uppercase tracking-[0.2em] text-flashdark">
          Thema {thema?.id ?? 6} · Bedrijfseconomische kern
        </span>
        ...
        {aantalBeantwoord === totaalVragen && (
          <div className="rounded-xl border border-border bg-header p-6 text-white">
            ...
            <p className="mt-1 text-sm text-slate-300">
              Onthoud: constante kosten lopen altijd door, variabele kosten bewegen mee
              met de verkoop, en pas ná het break-evenpunt gaat elke extra verkoop
              écht winst opleveren. Klik op "Opnieuw beginnen" om de quiz nog eens
              te maken.
            </p>
          </div>
        )}
