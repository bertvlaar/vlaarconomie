// src/pages/Thema7.jsx
//
// Pagina-component voor Thema 7 — Pretparken & Dynamic Pricing: Waarom kost
// een kaartje soms meer? (Prijsbeleid & Marktvraag).
// Wordt gerenderd binnen <ThemaLayout /> (zie App.jsx: route "/thema-7").
// Haalt alle inhoud op uit src/data/thema7Data.js en toont daarnaast, indien
// beschikbaar, de context/koppeling uit src/data/themas.js via useOutletContext.

import { useMemo, useState } from 'react'
import { Link, useOutletContext } from 'react-router-dom'
import thema7Data from '../data/thema7Data.js'

export default function Thema7() {
  const outletContext = useOutletContext() || {}
  const { thema } = outletContext

  const { titel, beschrijving, paragrafen, begrippen, oefenvragen } = thema7Data
    ...
    <section className="space-y-10 pb-16">
      ...
      <div>
        <span className="font-mono text-xs font-semibold uppercase tracking-[0.2em] text-flashdark">
          Thema {thema?.id ?? 7} · Prijsbeleid & Marktvraag
        </span>
        ...
        {aantalBeantwoord === totaalVragen && (
          <div className="rounded-xl border border-border bg-header p-6 text-white">
            ...
            <p className="mt-1 text-sm text-slate-300">
              Onthoud: vast aanbod plus wisselende vraag vraagt om een prijs die
              meebeweegt. Hoe inelastischer de vraag, hoe minder bezoekers een
              hogere prijs afschrikt — en hoe beter dynamic pricing helpt om
              piekbelasting te beperken en capaciteit te benutten. Klik op
              "Opnieuw beginnen" om de quiz nog eens te maken.
            </p>
          </div>
        )}
      </div>
